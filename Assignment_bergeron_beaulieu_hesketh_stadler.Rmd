...
# Short description
When researchers are doing taxonomic identification, they focus on the smallest taxonomic level the ID and write it in the datafile. Taxonomic identification can be long and laborious and can even be done on the field (eg. birds, fish and vegetation). It is eften difficult or even impossible to write all the higher level taxonomic information at the moment of the identification, even if this information is often usefull. To complete higher level taxonomic information, we used the database NCBI to find the missing higher level taxonomic information based on the lowest level IDed by the researchers (Spicified in a column). To do so, we created a data frame with the missing informations for each row, find the information in the database using the function tax_names from the package taxize. The missing information is then placed back in the data frame. In this particular case, the lowest level identified is not the same for all toxon and a column with the name of the lowest level identified taxon for a row is needed. A column for each higher level taxonomic level is also needed with names that correspond to the data base used.
...
## Retrieve higher level taxonomic information missing for each row by 

# Steps:
1) identify missing taxonomic information (NA) for each row, 2) create a dataframe with each row as the name of the taxonomic level missing, 3) find the missing taxonomic information in NCBI database using tax_name function, 4) replace the missing information with the information already present.
```{r}
